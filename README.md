...
- Fila de validação técnica (NOC) e fila de **Fusões Recusadas pelo GEOGRID** — quando a sala técnica recusa um item por dado confuso/incompleto (ver `geogrid.html`), ele aparece aqui com o motivo da recusa e o mesmo editor visual de fusão de `tecnico.html` (Lado A — Splitter 1 — Splitter 2 — Lado B, todos opcionais), pro líder corrigir e reenviar pro GEOGRID (volta pendente) ou excluir de vez. Editar aqui só corrige a cópia operacional da fila do GEOGRID, não reescreve o RFO original do técnico. A fila de aprovação de LPU (despacho) migrou pra `aprovacao_lpu.html` — ver abaixo.
...
- **Concluídos (histórico)**: fechado por padrão (clique no cabeçalho ou no botão ▼ pra expandir) — tende a acumular muitos itens.
