...
- Quadro (kanban) com uma coluna por Tipo de Solicitação (ou só a selecionada), cada uma com as atividades **do dia** (Timestamp Recebido de hoje), qualquer status — inclusive já finalizadas. Cada card mostra Cliente, Serviço/Atividade (Categoria 1-4 da `MATRIZ_SLA`, mesma classificação do despacho), Equipe despachada (Empresa/Técnico, com Apoio quando houver), Prazo Limite/atraso, os 4 horários (Despacho/Aceite/Chegada Prevista/Chegada Real) e o Status com uma bolinha colorida (âmbar=aguardando, verde=em deslocamento/campo, verde-claro=finalizado, vermelho=devolvido pro técnico).
- **Status de M.O — O&M**: rosca (donut) com o total de técnicos cadastrados (`LISTAR_TECNICOS`) hoje, divididos em Em Atividade (tem uma atividade em campo agora — mesma definição de "em andamento" de `index.html`), Ocioso, Treinamento, Férias e Indisponível (agrupa Inativo/Suspensão Disciplinar/Atestado/Manutenção de Frota da aba de Disponibilidade — ver `index.html`).
- **Resumo de Status — Atividades do Dia**: gráfico de barras com a contagem do dia em cada etapa do pipeline (Aguardando Despacho → Aguardando Início → Em Deslocamento → Em Campo → Aguardando Validação → Finalizado), mais uma barra à parte pra Devolvido (exceção, não é uma etapa do fluxo normal).
